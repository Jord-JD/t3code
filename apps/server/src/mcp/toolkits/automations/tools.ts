...
export const AutomationsToolkit = Toolkit.make(
  ...
  Tool.make("save_automation", {
    description:
      "Create or update a recurring automation for this project when requested by the user or when recurring follow-up would clearly help achieve their current goal. Check existing automations first to avoid duplicates. Choose a useful cadence and tell the user what you scheduled. Optionally set modelSelection (provider instance, model and options such as reasoning effort) and runtimeMode (permissions). On creation, an omitted model inherits from the current thread and omitted permissions default to full-access. Omitted settings remain unchanged on updates. Supply a stable id for updates, an IANA timezone and an RFC 5545 RRULE. Set continueThread to return to this conversation; otherwise each run creates a new thread. Continuing a conversation uses its model restrictions: keep the same provider driver and compatible resume state, and keep the model when the provider requires a new thread to change it. Model options remain editable. Skills may be referenced in the prompt.",
